/*
 * Vendored from FreeRDP 3.15.0, channels/rdpdr/server/rdpdr_main.c
 * (Apache License 2.0, see LICENSE.Apache-2.0 in this directory).
 *
 * Local changes for weston-mirror, marked "weston-mirror fix":
 *  - exported functions renamed to weston_rdpdr_server_context_new/_free
 *    so they do not clash with libfreerdp-server
 *  - write completion parsing fixed (upstream kills the channel on every
 *    non-empty write)
...
/* weston-mirror fix: requests are issued from several threads (FUSE workers
 * and this channel's thread); a plain ++ handed out duplicate completion
 * ids and responses were delivered to the wrong request. */
static UINT32 rdpdr_next_completion_id(RdpdrServerContext* context)
{
	return __atomic_fetch_add(&context->priv->NextCompletionId, 1, __ATOMIC_SEQ_CST);
}
                                                          ...
                                                          const RDPDR_HEADER* header)
{
	...
	for (UINT16 i = 0; i < numCapabilities; i++)
	{
		...
		/* weston-mirror fix: upstream OR-ed the capability *type* (1..5)
		 * into a mask that is compared with RDPDR_DTYP_* bits, so e.g.
		 * the drive capability (4) counted as PRINT and FILESYSTEM was
		 * never seen. Map the types to the device type bits. */
		switch (capabilityHeader.CapabilityType)
		{
			case CAP_PRINTER_TYPE:
				caps |= RDPDR_DTYP_PRINT;
				break;
			case CAP_PORT_TYPE:
				caps |= RDPDR_DTYP_SERIAL | RDPDR_DTYP_PARALLEL;
				break;
			case CAP_DRIVE_TYPE:
				caps |= RDPDR_DTYP_FILESYSTEM;
				break;
			case CAP_SMARTCARD_TYPE:
				caps |= RDPDR_DTYP_SMARTCARD;
				break;
			default:
				break;
		}
	...
	/* weston-mirror fix: evaluate once all capabilities are known;
	 * upstream did this inside the loop and dropped support for every
	 * device type whose capability came later in the PDU. */
...
static DWORD WINAPI rdpdr_server_thread(LPVOID arg)
{
	...
	while (1)
	{
		...
		if (BytesReturned >= RDPDR_HEADER_LENGTH)
		{
			...
			/* weston-mirror fix: a virtual channel message carries exactly
			 * one RDPDR PDU. Upstream kept parsing whatever a handler left
			 * unread (padding, e.g. of DR_CLOSE_RSP) as the next PDU and
			 * killed the channel on "Unknown RDPDR_HEADER.Component". */
                                                          ...
                                                          UINT32 completionId, UINT32 ioStatus)
{
	...
	/* Setup the IRP. */
	irp->CompletionId = rdpdr_next_completion_id(context);
                                                         ...
                                                         UINT32 completionId, UINT32 ioStatus)
{
	...
	{
		/* weston-mirror fix: padding optional */
                                                         ...
                                                         UINT32 completionId, UINT32 ioStatus)
{
	...
	irp->DeviceId = deviceId;
	irp->FileId = fileId;
	/* weston-mirror fix: "*" instead of "*.*". Windows treats both alike,
	 * but FreeRDP/WinPR clients match "*.*" literally and hide every
	 * name without a dot (most directories). */
                                                   ...
                                                   UINT32 completionId, UINT32 ioStatus)
{
	...
	/*
	 * weston-mirror fix: DR_WRITE_RSP (MS-RDPEFS 2.2.1.5.4) is Length plus
	 * an optional padding byte. Length is the number of bytes written,
	 * the response carries no data. Upstream required Length further
	 * bytes, failed on every non-empty write and killed the channel
	 * thread.
	 */
	if (!Stream_CheckAndLogRequiredLengthWLog(context->priv->log, s, 4))
		return ERROR_INVALID_DATA;
	...
	Stream_Read_UINT32(s, length); /* Length (4 bytes) */
	if (Stream_GetRemainingLength(s) >= 1)
		Stream_Seek(s, 1);     /* Padding (1 byte, optional) */
                                                     ...
                                                     UINT32 completionId, UINT32 ioStatus)
{
	...
	Stream_Read_UINT32(s, length); /* Length (4 bytes) */
	/* weston-mirror fix: the padding byte is optional */
	if (Stream_GetRemainingLength(s) >= 1)
		Stream_Seek(s, 1);             /* Padding (1 byte) */
